...
const StyleSelector = ({value, onChange, isOpen, setIsOpen} : {value: ThumbnailStyle; onChange: (style: ThumbnailStyle) => void; isOpen: boolean; setIsOpen: (open: boolean) => void}) => {
   ...
   const styleDescriptions: Record<ThumbnailStyle, string> = {
    ...
    'Minimalist': 'Clean design with ample white space, simple typography, and a limited color palette.',
    'Photorealistic': 'Lifelike images with attention to detail, realistic lighting, and textures.',
    'Illustrated': 'Hand-drawn or digitally created illustrations with a unique artistic style.',
    'Tech & Futuristic': 'Sleek, modern design with metallic colors, neon accents, and tech-inspired elements.',
  ...
  return (
    <div className="relative space-y-3 dark">
      <label className="block text-sm font-medium text-zinc-200">Thumbanail Style</label>
          ...
          {thumbnailStyles.map((style) => (
            ...
            className=" flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-400">{styleDescriptions[style]}</p>
              </div>
