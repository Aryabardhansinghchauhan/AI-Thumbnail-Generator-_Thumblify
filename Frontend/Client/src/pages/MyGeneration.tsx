    ...
    <>
      ...
      <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-zinc-400 mt-1">View and manage your generated thumbnails</p>
        </div>
          {/*LOADING */}
          {loading && (
          ...
          <p className="text-sm text-zinc-400 mt-2">Generate your first thumbnail to it here</p>
            </div>
          )}
          {/*Thumbnails Grid */}
          {!loading && thumbnails.length > 0 && (
                ...
                return(
                  <div key={thumb._id} onClick={()=> navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                    ...
                    <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                      {thumb.image_url ? (
                        <img src={thumb.image_url} alt={thumb.title} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300"/>
                      ):(
                      ...
                      {thumb.isGenerating && <div className="absolute insert-0 bg-black/50 flex items-center justify-center text-m font-medium text-white">Generating...</div>}
                    </div>
                    ...
                    <div className="p-4 space-y-2">
                      ...
                      <p className="text-us text-zinc-500">{new Date(thumb.createdAt!).toDateString()}</p>
                      </div>
